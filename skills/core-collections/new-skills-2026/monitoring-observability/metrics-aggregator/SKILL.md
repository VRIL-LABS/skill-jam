...
description: Aggregate and process metrics — Prometheus, StatsD, or InfluxDB. Use when collecting custom metrics, aggregating data, or implementing metric pipelines.
---
...
Invoke this skill when you need to work with metrics aggregator in your infrastructure or application.

## Quick Start

```bash
# Example command or configuration
echo "Sample usage for metrics-aggregator"
```
...
Description of a basic use case for metrics-aggregator.
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Configure metrics-aggregator
        run: |
          # Setup commands
          echo "Configuring metrics-aggregator"
```
