---
name: sse-handler
description: Implement Server-Sent Events — one-way real-time updates. Use when streaming updates, implementing notifications, or server push.
---
...
Invoke this skill when you need to work with sse handler in your infrastructure or application.

## Quick Start

```bash
# Example command or configuration
echo "Sample usage for sse-handler"
```
...
Description of a basic use case for sse-handler.
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Configure sse-handler
        run: |
          # Setup commands
          echo "Configuring sse-handler"
```
