...
description: Manage Azure Blob Storage — containers, access tiers, and lifecycle. Use when managing Azure storage, implementing policies, or organizing blobs.
---
...
Invoke this skill when you need to work with blob storage manager in your infrastructure or application.

## Quick Start

```bash
# Example command or configuration
echo "Sample usage for blob-storage-manager"
```
...
Description of a basic use case for blob-storage-manager.
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Configure blob-storage-manager
        run: |
          # Setup commands
          echo "Configuring blob-storage-manager"
```
