...
description: Profile memory usage — leak detection and optimization. Use when investigating memory issues, optimizing memory, or finding leaks.
---
...
Invoke this skill when you need to work with memory profiler in your infrastructure or application.

## Quick Start

```bash
# Example command or configuration
echo "Sample usage for memory-profiler"
```
...
Description of a basic use case for memory-profiler.
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Configure memory-profiler
        run: |
          # Setup commands
          echo "Configuring memory-profiler"
```
